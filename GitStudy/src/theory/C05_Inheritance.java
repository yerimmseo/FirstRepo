...
//	}
	...
	void sayHi() {
		...
		if (age < 0) {
			...
			System.out.printf("안녕! 내 이름은 %s야. 나이는 %d야.\n", name, age);
		} else if (age < 20) {
			System.out.printf("안녕하세요! 제 이름은 %s. 나이는 %d살입니다.\n", name, age);
		} else {
...
/*
	1. 상속받는 클래스는 생성자에서 가장 먼저 부모의 생성자를 호출해야 한다.
	   부모 클래스에 기본생성자가 존재하면 생략할 수 있지만, 존재하지 않을 때는 생략이 불가능하다.
...
class Police extends Person {
	...
	// 자식클래스의 생성자에서는 첫 줄에 super()를 반드시 호출해야한다. (필수)
	public Police() {
		super("김경찰", 23, 20);
		// super(): 부모클래스의 생성자를 호출할 때 사용한다.
	}
	...
	void arrest(String suspect) {
		System.out.printf("%s형사는 용의자 %s씨를 체포했습니다.\n", name, suspect);
	}
}

//class Programmer extends Person {
//	// 자식 클래스에서 생성자를 정의하지 않을 시 눈에는 보이지 않지만
